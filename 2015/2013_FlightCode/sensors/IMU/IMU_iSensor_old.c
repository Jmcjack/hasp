 ...
 *
 * $Id: IMU_iSensor.c 756 2012-01-04 18:51:43Z murch $
 */
...
int read_imu_time(struct xray *xrayData_ptr){
	xrayData_ptr->time = get_Time();
	return 0;
}
int read_imu(struct imu *imuData_ptr) {
	...
	HAL_WRITE_UINT32(MPC5XXX_PSC1+MPC5XXX_PSC_CR,0x05);
	...
	/* cyg_uint32 dr0;
	cyg_uint32 dr1;
	HAL_WRITE_UINT32(0xF0000F00,0x40000000|0x10000000);		// enable SPI
	
	HAL_WRITE_UINT32(0xF0000F09,0x80000000|0x40000000|0x20000000|0x10000000);	// write to g-data register
	
	cyg_uint32 status;
	HAL_READ_UINT32(0xF0000F05,status);
	status<<=24;
	...
	HAL_READ_UINT32(0xF000C200,dr0);
	dr0 <<= 24;
	...
	HAL_READ_UINT32(0xF000C200,dr1);
	
	dr1 <<=16;
	
	printf("--%d 1--%d 2--", dr0,dr1); */
