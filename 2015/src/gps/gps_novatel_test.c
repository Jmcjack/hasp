...
int init_GPS()
{
        ...
        if(tcgetattr(fd, &uart5) < 0) {
                fprintf(stderr,"Could not get attributes of UART5 at usb0\n");}
...
int read_GPS(fd)
{
	...
	bytesInLocalBuffer = read(fd, &response[0],512);
	...
	while(counter<bytesInLocalBuffer){
		...
		//		printf("GPS FLAG1\n");
			...
			Xe = *((double *)(&response[36]));
			Ye = *((double *)(&response[44]));
			Ze = *((double *)(&response[52]));
			
			fprintf(stderr,"X-pos = %d", Xe);
			fprintf(stderr,"Y-pos = %d", Ye);
			fprintf(stderr,"Z-pos = %d", Ze);
