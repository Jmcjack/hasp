 ...
 *
 * $Id: gps_crescent.c 773 2012-02-20 19:55:36Z lie $
 */
...
switch(localBuffer[5]*256 + localBuffer[4] ){
	...
	case 1: // parse msg 1 
		...
		if((*((uint16_t *)(&localBuffer[56]))) != 0) {// here, 0 means no fix. for SirfIII, 0 means fix
			if(fabs(gpsData_ptr->GPS_TOW - floor(gpsData_ptr->GPS_TOW)) < 1e-3) {
				// Added another level of protection, GPS TOW needs to be fine-steered to a whole number of TOW.
