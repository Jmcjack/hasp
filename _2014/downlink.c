...
void send_telemetry(struct sensordata *sensorData)
{
	...
	//countsA
	sprintf(string_to_write,"%d,",sensorData->xrayData_ptr->countsA);
	memcpy(&sendpacket[36],string_to_write,9);
        reset_string(string_to_write);	
	//countsB
	
	sprintf(string_to_write,"%d,",sensorData->xrayData_ptr->countsB);
	memcpy(&sendpacket[45],string_to_write,9);
	reset_string(string_to_write);
         
	//countsAB
	sprintf(string_to_write,"%d,",sensorData->xrayData_ptr->countsAB);
