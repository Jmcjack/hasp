...
cyg_uint32			byte0;			// Data byte 0
cyg_uint32			byte1;			// Data byte 1
...
get_peak_data(struct xray *xrayData_ptr)
{
	...
	xrayData_ptr->peak = byte0+byte1;		// store *previous* data in the xray logger pointer
...
{
    cyg_addrword_t data = 0;		// We aren't passing any data through
